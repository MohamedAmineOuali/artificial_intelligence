...
def choix_base():
    BC_number = input(color_schema%('' ,'Choisir la base de connaissances 1 ou 2 : '))
    ...
    BF_number = int(input(color_schema%('' ,'Choisir la base des faits : '))) - 1
...
goal=input(color_schema%('' ,'Choisir le but à chercher(appuyer entrer pour sauter cette étape) : '))
...
if goal in BF:
    print(color_schema%('' ,"but trouvé"))


trace(BR,activated_rules_order,sys.stdout)

save=int(input(color_schema%('' ,'voulez vous enregistrer la trace d\'execution:\n'
                 '1 : yes'
                 '2 : no \n')))
if(save==1):
    filename=input(color_schema%('' ,"entrer le nom du fichier"))
